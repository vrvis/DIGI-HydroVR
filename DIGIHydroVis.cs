...
{
    [ExecuteInEditMode]
    public class DIGIHydroVis : AbstractVisualisation
    {
        [SerializeField]
        [HideInInspector]
        public Vector3 visPosition = Vector3.zero;
        ...
        public override void CreateVisualisation()
        {
            ...
            creationConfiguration.VisualisationType = AbstractVisualisation.VisualisationTypes.DIGIHYDRO_FLOW;
            ...
            if (view != null)
            {
                view.transform.localPosition = visPosition;
        ...
        private void AdjustViewPosition(Vector3 newPosition)
        {
            foreach(View view in viewList)
            {
                view.gameObject.transform.localPosition = newPosition;
            }
        }
            ...
            if (visualisationReference.dataSource != null)
            {
                    ...
                    (visualisationReference.dataSource[getAxis(configuration.Axies, CreationConfiguration.Axis.Z)] != null))
                {
                    ...
                    if (visualisationReference.dataSource[getAxis(configuration.Axies, CreationConfiguration.Axis.X)] != null)
                    {
                        ...
                        float[] minMax_X = new float[2];
                        minMax_X = PosScaleUtil.GetMaxAndMinValues(visualisationReference.dataSource, "X");
                        float scaleX = PosScaleUtil.FindScalingFactor(minMax_X[0], minMax_X[1]);
                        visualisationReference.width = scaleX;
                        visPosition.x = minMax_X[0];
                    ...
                    if (visualisationReference.dataSource[getAxis(configuration.Axies, CreationConfiguration.Axis.Y)] != null)
                    {
                        ...
                        float[] minMax_Y = new float[2];
                        minMax_Y = PosScaleUtil.GetMaxAndMinValues(visualisationReference.dataSource, "Y"); //unity has swaped Y and Z compared to the original data source
                        float scaleZ = PosScaleUtil.FindScalingFactor(minMax_Y[0], minMax_Y[1]);
                        visualisationReference.depth = scaleZ;
                        visPosition.z = minMax_Y[0];
                    }
                    if (visualisationReference.dataSource[getAxis(configuration.Axies, CreationConfiguration.Axis.Z)] != null)
                    {
                        ...
                        float[] minMax_Z = new float[2];
                        minMax_Z = PosScaleUtil.GetMaxAndMinValues(visualisationReference.dataSource, "Z");
                        float scaleY = PosScaleUtil.FindScalingFactor(minMax_Z[0], minMax_Z[1]);//unity has swaped Y and Z compared to the original data source
                        visualisationReference.height = scaleY;
                        visPosition.y = minMax_Z[0];
                    }
